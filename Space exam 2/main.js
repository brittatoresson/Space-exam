...
async function getData() {
  //Testa för fel vid hämtning av data
  try {
    ...
    return await data;
    //Vid fel i Try => hanterar error och visar på display via alert
  } catch (error) {
...
function searchGalaxy(data, inputValue) {
  ...
  for (let i = 0; i < data.bodies.length; i++) {
    ...
    //Kalla på funktion för att addera IMG till objektet
    addImgToObj(element);
    ...
    // Loopa igenom element.moons, jämför input med moon-arrayen
    for (let i = 0; i < element.moons.length; i++) {
      let moons = element.moons[i];
      //Ta bort två "titan"-månar från saturnus-arrayen
      if (inputValue.toLowerCase() === "Titan".toLowerCase()) {
        element.moons.splice(1, 1);
      }
      // Om input = månnamn
